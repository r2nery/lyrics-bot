...
class CropAudio:
    ...
    def crop(self):
        ...
        if self.out_dir is not None:
            ...
            outfile = self.out_dir / f"{self.audio_file.stem}.{self.format}"
...
if __name__ == "__main__":
    cwd = Path.cwd()
    print(cwd)
    file = cwd / "0.downloads" / "Jessie Ware - Spotlight (Official Music Video).mp3"
    out = cwd / "1.crops"

    ca = CropAudio(file, out, verbose=True).crop()
